import{c as a,Q as n,j as p,m as i}from"./chunks/framework.CBiVa4O3.js";const k=JSON.parse('{"title":"浮点格式全景:精度、范围、显存的三角","description":"","frontmatter":{},"headers":[],"relativePath":"../aiInfraLearning/04-浮点格式全景.md","filePath":"../aiInfraLearning/04-浮点格式全景.md","lastUpdated":1778649484000}'),l={name:"../aiInfraLearning/04-浮点格式全景.md"};function e(t,s,h,o,r,c){return n(),p("div",null,[...s[0]||(s[0]=[i(`<h1 id="浮点格式全景-精度、范围、显存的三角" tabindex="-1">浮点格式全景:精度、范围、显存的三角 <a class="header-anchor" href="#浮点格式全景-精度、范围、显存的三角" aria-label="Permalink to &quot;浮点格式全景:精度、范围、显存的三角&quot;">​</a></h1><p>LLM Infra 的所有优化最终都要回答一个问题:这个数用几位存,用什么格式存。每砍一半精度,显存砍半、HBM 带宽利用率翻倍、Tensor Core 算力翻倍——这是 LLM 工程链上唯一一个能&quot;一刀三吃&quot;的杠杆。所以 FP32 → FP16 → BF16 → FP8 → INT4 这条路不是炫技,是过去十年把模型从 BERT 推到 405B 的经济学基础。这一篇把每个格式的位分布、表达范围、精度损失、硬件支持画清楚,后面 14 / 22 / 23 篇讲 ZeRO / FP8 训练 / KV 量化时,所有结论都站在这一篇上。</p><blockquote><p>一句话先记住:<strong>FP32 占 4 字节、FP16/BF16 占 2、FP8 占 1、INT4 占 0.5;同样 16 位,FP16 多 3 位精度但少 5 位指数,BF16 反之——训练用 BF16 是因为指数范围决定数值稳定;FP8 必须区分 E4M3(forward,精度高)和 E5M2(backward,范围大);INT8/INT4 是定点数,要 scale + zero point,本质和浮点不是一种东西</strong>。</p></blockquote><hr><h2 id="一、为什么浮点格式是-llm-infra-第一道选型" tabindex="-1">一、为什么浮点格式是 LLM Infra 第一道选型 <a class="header-anchor" href="#一、为什么浮点格式是-llm-infra-第一道选型" aria-label="Permalink to &quot;一、为什么浮点格式是 LLM Infra 第一道选型&quot;">​</a></h2><p>把&quot;格式占几位&quot;这件事的工程后果摆出来:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>70B 模型权重在不同格式下的显存:</span></span>
