import{c as a,Q as n,j as p,m as e}from"./chunks/framework.Bhbi9jCp.js";const h=JSON.parse('{"title":"RED 与 USE 方法:服务该看什么 / 资源该看什么 / 两者不能混","description":"","frontmatter":{},"headers":[],"relativePath":"devopsLearning/14-RED与USE方法.md","filePath":"devopsLearning/14-RED与USE方法.md","lastUpdated":1779604575000}'),l={name:"devopsLearning/14-RED与USE方法.md"};function t(i,s,o,c,r,u){return n(),p("div",null,[...s[0]||(s[0]=[e(`<h1 id="red-与-use-方法-服务该看什么-资源该看什么-两者不能混" tabindex="-1">RED 与 USE 方法:服务该看什么 / 资源该看什么 / 两者不能混 <a class="header-anchor" href="#red-与-use-方法-服务该看什么-资源该看什么-两者不能混" aria-label="Permalink to &quot;RED 与 USE 方法:服务该看什么 / 资源该看什么 / 两者不能混&quot;">​</a></h1><p>上一篇 13 讲 SLI/SLO/SLA——SLI 是定义、SLO 是承诺、SLA 是合同。但定义说完,真到值班那一刻,<strong>你打开 Grafana 看哪几个 panel?</strong> 这就是 14 篇要解决的问题。<strong>前面讲的是&quot;该承诺什么&quot;,这一篇讲的是&quot;该看什么&quot;</strong>——两件事都做不好,告警是配不出来的,因为你根本不知道该把告警挂在哪个指标上。</p><blockquote><p>一句话先记住:<strong>RED 是给服务的体温表,USE 是给资源的血压计——一个测&quot;用户感觉怎么样&quot;,一个测&quot;机器还撑得住吗&quot;,混用就像拿温度计量血压一样荒唐</strong>。我见过太多团队把 CPU &gt; 80% 的告警当成&quot;服务变慢了&quot;——结果业务延迟从 100ms 涨到 2s 那一刻,CPU 才 30%,告警一声不响。<strong>那 90% 的 CPU 是 GC 在原地烧,不是用户请求在烧</strong>。这一篇讲清楚:<strong>用户视角先看 RED 确认有没有事,机器视角再看 USE 找根因</strong>——顺序反了一次,事故就拖一倍。</p></blockquote><hr><h2 id="一、问题场景-看错指标的两种悲剧" tabindex="-1">一、问题场景:看错指标的两种悲剧 <a class="header-anchor" href="#一、问题场景-看错指标的两种悲剧" aria-label="Permalink to &quot;一、问题场景:看错指标的两种悲剧&quot;">​</a></h2><p>我把生产事故里&quot;指标用错&quot;的场景归成两类——<strong>漏报和误报</strong>,每类都见过几十次。</p><h3 id="_1-1-悲剧-a-用资源指标告警-漏报了服务问题" tabindex="-1">1.1 悲剧 A:用资源指标告警,漏报了服务问题 <a class="header-anchor" href="#_1-1-悲剧-a-用资源指标告警-漏报了服务问题" aria-label="Permalink to &quot;1.1 悲剧 A:用资源指标告警,漏报了服务问题&quot;">​</a></h3><p>某团队订单服务,告警配的是「CPU &gt; 80% 持续 5min」。某天凌晨业务延迟从 80ms 飙到 1.8s,<strong>告警一声不响</strong>——因为 CPU 才 45%。</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>凌晨 1:30  用户开始反馈下单慢</span></span>
...
<span class="line"><span>   - 走对了顺序:先 RED 确认&quot;用户感觉怎么样&quot;,再下钻</span></span></code></pre></div><h3 id="_2-3-red-在-prometheus-里的标准写法" tabindex="-1">2.3 RED 在 Prometheus 里的标准写法 <a class="header-anchor" href="#_2-3-red-在-prometheus-里的标准写法" aria-label="Permalink to &quot;2.3 RED 在 Prometheus 里的标准写法&quot;">​</a></h3><p>针对一个 HTTP 服务,<strong>这三条 PromQL 几乎是固定写法</strong>:</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span># R: 当前 5min QPS</span></span>
...
<span class="line"><span>└────────────────┴──────────────────────┴───────────────────────┴───────────────────────┘</span></span></code></pre></div><p><strong>这张表回答你 90% 的&quot;该看哪个指标&quot;的问题</strong>。需要新建一个监控面板?对照这张表,<strong>每个资源三列都要有</strong>——少一列就少了一个看见故障的眼睛。</p><h3 id="_3-4-use-在-node-exporter-的常见查询" tabindex="-1">3.4 USE 在 node_exporter 的常见查询 <a class="header-anchor" href="#_3-4-use-在-node-exporter-的常见查询" aria-label="Permalink to &quot;3.4 USE 在 node_exporter 的常见查询&quot;">​</a></h3><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span># CPU U:  CPU 利用率(1 - idle)</span></span>
