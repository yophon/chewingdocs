...
<span class="line"><span></span></span>
<span class="line"><span>Alt-f    →  toggle floating(不需要进模式,直接切)</span></span></code></pre></div><p><strong>模式 vs prefix 的本质差异</strong>:</p><ul><li>tmux:<code>prefix + 字母</code> = 两次按键,<strong>每次都要重按 prefix</strong></li><li>Zellij:<strong>进入模式后停留</strong>,直到 ESC 退出,<strong>模式内连续操作只按一个字母</strong></li></ul><p><strong>适用场景的差异</strong>:</p><ul><li>你想切 1 个 pane:tmux 和 Zellij 都是 2 步,差不多</li><li>你想连续切 4 个 pane:tmux 是 <code>prefix h prefix h prefix h prefix h</code>(8 步),Zellij 是 <code>Ctrl-p h h h h ESC</code>(6 步)</li><li>你想 split + 切 + split:tmux 是 <code>prefix | prefix l prefix -</code>(6 步),Zellij 是 <code>Ctrl-p n l n ESC</code>(5 步)</li></ul><p><strong>模式的好处</strong>:连续操作时按键少;<strong>坏处</strong>:你要记得&quot;现在在哪个模式&quot;,ESC 漏按会按错键。<strong>两边各有取舍</strong>。</p><h3 id="_3-4-第一份-config-zellij-config-kdl" tabindex="-1">3.4 第一份 ~/.config/zellij/config.kdl <a class="header-anchor" href="#_3-4-第一份-config-zellij-config-kdl" aria-label="Permalink to &quot;3.4 第一份 ~/.config/zellij/config.kdl&quot;">​</a></h3><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// ~/.config/zellij/config.kdl</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p><strong>比 JSON 多了注释和无引号字符串,比 YAML 多了结构化嵌套,比 TOML 多了表达力</strong>。Zellij 选 KDL 是有道理的:<strong>multiplexer 的配置本质上是嵌套结构(layout 嵌套 layout,pane 嵌套 pane),KDL 比 YAML 表达这种嵌套更清楚</strong>。</p><h3 id="_4-2-一份完整的-layout-文件" tabindex="-1">4.2 一份完整的 layout 文件 <a class="header-anchor" href="#_4-2-一份完整的-layout-文件" aria-label="Permalink to &quot;4.2 一份完整的 layout 文件&quot;">​</a></h3><p>把 nvim + cargo run + cargo watch test + Claude Code 跑在一个 layout 里:</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// ~/.config/zellij/layouts/dev.kdl</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">zellij</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> --layout</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> ~/.config/zellij/layouts/dev.kdl</span></span></code></pre></div><p><strong>一行命令,工作台就位</strong>——nvim 在左、cargo run 跑在右上、cargo watch test 跑在右下、顶部 tab 栏、底部 status 栏。<strong>这是 tmuxinator 在 tmux 里能勉强做到的事,但 Zellij 是原生</strong>。</p><h3 id="_4-3-这份-layout-的每段在做什么" tabindex="-1">4.3 这份 layout 的每段在做什么 <a class="header-anchor" href="#_4-3-这份-layout-的每段在做什么" aria-label="Permalink to &quot;4.3 这份 layout 的每段在做什么&quot;">​</a></h3><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>layout {                              // 整个 layout 的根</span></span>
...
<span class="line"><span>    plugin location=&quot;zellij:status-bar&quot;</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span>}</span></span></code></pre></div><p><strong>核心概念</strong>:</p><ul><li><code>pane</code> 是一个矩形区域,可以是终端(有 command)或插件(有 plugin)</li><li><code>split_direction=&quot;vertical&quot;</code> = 左右分;<code>&quot;horizontal&quot;</code> = 上下分</li><li><code>size</code> 可以是百分比(<code>&quot;60%&quot;</code>)或固定行数(<code>size=1</code>)</li><li><code>borderless=true</code> 表示这个 pane 不要边框(用于状态栏)</li><li>嵌套 pane 实现复杂 layout</li></ul><h3 id="_4-4-多个-tab-的-layout" tabindex="-1">4.4 多个 tab 的 layout <a class="header-anchor" href="#_4-4-多个-tab-的-layout" aria-label="Permalink to &quot;4.4 多个 tab 的 layout&quot;">​</a></h3><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>// 三个 tab:dev / logs / db</span></span>
<span class="line"><span>layout {</span></span>
